...
I am not a security expert, and security doesn't come by default in these situations. So I caution you not to use untrusted input in the commands fed to zsh. Nevertheless, the only threat I can envision is the input containing Brish's EOE marker, as otherwise the input gets automatically escaped. I plan to add checks for this in future versions. Feedback by security experts will be appreciated.
